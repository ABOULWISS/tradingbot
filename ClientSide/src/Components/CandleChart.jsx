...
export default function CandleChart({ onReady   ,  timeframe, onTimeframeChange}) {
  ...
  // ======================
  // FETCH HISTORICAL DATA (BY TIMEFRAME)
  // ======================
  useEffect(() => {
  ...
  // ======================
  // LIVE WEBSOCKET UPDATES (PER TIMEFRAME)
  // ======================
  useEffect(() => {
    ...
    ws.onmessage = e => {
      ...
      const candle = {
        time: Number(c.time),
        open: toSafeNumber(c.open),
        high: toSafeNumber(c.high),
        low: toSafeNumber(c.low),
        close: toSafeNumber(c.close),
      };
